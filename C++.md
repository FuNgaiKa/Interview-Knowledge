*cpp篇：2*
STL：
vector的扩容 它是一个动态数组 底层为连续的线性内存空间
...
![](media/16157979393781.jpg)


![](media/16157980296166.jpg)
...
构造函数为什么一般不定义为虚函数？
1.实例化对象才可以调用虚函数。此时对象都还没有创建，虚函数表存在，但是虚函数的指针还不存在 2. 虚函数本来是基类的指针指向派生类的对象时，通过该指针实现对派生类的虚函数调用，而构造函数是创建对象时自动调用的，这两者不同。
...
C11新特性：lambda表达式 模版 泛型编程（通过模版） 智能指针
...
cpp是不是类型安全的？不是，两个不同类型的指针可以进行强制转换。
